...
# ----------------------------------------
start_date = dt.datetime.now() - dt.timedelta(weeks=12)
...
for ticker in ipsa_stocks:
    ...
    if ticker not in all_data:
        results.append({"Stock": ticker, "Último Precio": None, "RSI": None, "P/E Ratio": None, "Debt/Equity": None})
        continue
    ...
    fm = safe_get_financial_metrics(ticker)
    stock_data = {
        "Stock": ticker,
        "Último Precio": last_close,
        "RSI": round(last_rsi, 2) if last_rsi is not None else None,
        "P/E Ratio": fm.get('pe_ratio'),
        "Debt/Equity": fm.get('debt_to_equity')
    }
